...
# 1. Ensure 24-bit uncompressed lena.bmp exists
python3 -c "
import struct
w, h = 256, 256
row_bytes = w * 3
...
# 2. Scope #pragma pack strictly in image.h
python3 -c "
...
# 3. Patch gui.c to assign absolute path to filename
python3 -c "
import re
...
for line in lines:
    ...
    if 'IupFileDlg' in s or 'IupPopup' in s or ('IupDestroy' in s and 'filedlg' in s):
        ...
        patched.append(indent + f'filename = \"{ABS_LENA}\";')
    elif 'char *filename' in s or 'char* filename' in s:
        indent = line[:len(line) - len(line.lstrip())]
        patched.append(indent + f'char *filename = \"{ABS_LENA}\";')
    else:
        patched.append(line)

with open('gui.c', 'w') as f:
    f.write('\n'.join(patched))
print('Assigned filename path in gui.c: $ABS_LENA')
"
