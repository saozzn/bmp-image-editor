...
# 1. Clean gui.c and replace hardcoded load with dynamic IupFileDlg
python3 -c "
with open('gui.c', 'r') as f:
    code = f.read()

# Remove all existing open_image_callback definitions to avoid duplicates
...
# Append clean dynamic open_image_callback using IUP file dialog
dynamic_callback = '''
...
with open('gui.c', 'w') as f:
    f.write(code)
print('[SUCCESS] Dynamic file dialog restored in gui.c')
"

# 2. Compile and launch application
